...
emptyCommentRow <- function(){
		if(is.null(tableValues$commentsTable)) return(NULL)
		if(ncol(tableValues$commentsTable)==3) return(NULL)
...
emptyContinuousVarsRow <- function(){
		if(is.null(tableValues$continuousVarsTable)) return(NULL)
		if(ncol(tableValues$continuousVarsTable)==3) return(NULL)
...
emptyScanVarsRow <- function(){
		if(is.null(tableValues$scanVarsTable)) {
			return(data.frame(empty="", stringsAsFactors=F))
			}
		if(ncol(tableValues$scanVarsTable)==4) {
			return(data.frame(empty="", stringsAsFactors=F))
			}
