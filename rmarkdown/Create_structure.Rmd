...
To run all the behaviors you wish to record during focal observations, you will need to prepare and upload ".csv" files using the panel on the left. This allows you to generate the **"behaviors.json"** file that is used by the app. The information included in this file will be displayed as pop-windows during a focal. A minimum of 3 files is needed to generate **"behaviors.json"**. 

1. Dyadic interactions, recorded between scans by turning 'move' off and dragging your finger between two individuals from the scene.
2. Animal activities, recorded during scans by tapping on the individuals or by tap-and-dragging your finger around a group of individuals.  
3. Self-directed/health information, recorded by double-tapping on an individual.
...
Important: if "**\***" is added at the end of a column header, the corresponding modifier will allow multiple levels to be selected.
...
* Note the presence of "**\***" at the end of the GROOMEE_ID column header. This allows muliple individual IDs to be selected simultaneously. The "**\***" symbol can be used for any column of the dyadic, scan and self directed/health csv files.
...
#### 6. Add behaviors.json file to iPad
...
Note that you can edit behaviors.json manually using any basic text editor or online using [jsoneditoronline.org](http://www.jsoneditoronline.org).
