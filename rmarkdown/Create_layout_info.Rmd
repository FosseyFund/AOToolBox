...
2. Focal variables, recorded at the beginning of a focal, when the focal individual is selected.  
3. Scan variables, recorded during scans, and corresponding to variables that must be recorded at fixed time intervals such as weather, temperature, group activity, vegetation type, ...etc.
...
If "*" is added at the end of the column header in one of the four ".csv" files, the corresponding modifier will allow multiple levels to be selected.
If a given modifier takes "_ID" as a possible value, then the user will be able to select one of the individuals of the selected groups. *N.B.*: do not use "_ID" for session variables, because social groups may not have been selected yet at this stage.

Here is an example of a table that can be saved as ".csv" file to customize continuous focal variables:
