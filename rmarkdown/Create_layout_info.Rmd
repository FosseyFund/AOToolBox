...
**Read instructions below, then <a id="link_to_sessionSetup" href="#" class="action-button">switch tab</a> and follow the steps on the left pannel.**
...
If "**\***" is added at the end of a column header, the corresponding modifier will allow multiple levels to be selected.
If a modifier takes "**\_ID**" as a possible value, then the user will be able to select one of the individuals of the selected groups. *N.B.*: do not use "**\_ID**" for session variables, because social groups may not have been selected yet at this stage.
