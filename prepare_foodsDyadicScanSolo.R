...
if(ncol(dyadic_fee2)==indexx){##last column is food
  dyadic_fee3 <- data.frame(dyadic_fee2[,1:(indexx-1)], foods[rep(1:nrow(foods), nrow(dyadic_fee)),, drop=FALSE], check.names=F)
  names(dyadic_fee3)[1:(indexx-1)]<-names(dyadic_fee2)[1:(indexx-1)]
} else {
dyadic_fee3 <- data.frame(dyadic_fee2[,1:(indexx-1)], foods[rep(1:nrow(foods), nrow(dyadic_fee)),, drop=FALSE],dyadic_fee2[,(indexx+1):ncol(dyadic_fee2)], check.names=F)
names(dyadic_fee3)[1:(indexx-1)]<-names(dyadic_fee2)[1:(indexx-1)]
